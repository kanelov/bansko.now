...
> | 4 · QR меню, `/places`, часове | готово | 2026-09-16 | QR менюто работи и при заключен сайт; часове + изключения; профил с откъс и JSON-LD; 308 от `/businesses`; помощ `/business/guide`; менютата влизат в картата на сайта |
> | 5 · Екрани `/display/<token>` | готово | 2026-09-16 | три шаблона, колони и таблица от варианти по правила, версия на 60 s през edge кеш, преглед 16:9 = същата страница, видео качва админът; страницата без Tailwind (Tizen) |
> | 6 · Печат | готово | 2026-09-17 | A4/A3/A5/DL, BG/EN/двуезично, фон от оригинала, режим за печатница (+3 mm), QR към актуалното меню; PDF от диалога за печат; без миграция |
> | 7 · Втвърдяване и помощ в портала | готово | 2026-09-17 | помощ в портала, управление от админа (`/admin/platform`), смяна на парола, публичната регистрация е изключена, мин. парола 10 знака, ограничаване на опитите за вход (8/30/60 за 15 мин., само отпечатъци), телевизорът проверен на живо; защитата от изтекли пароли иска Supabase Pro |
> | 8 · Пилот | предстои | | истинското кафе се въвежда преди пилота |
> | 8b · Къс адрес за телевизора, зелен QR, лента с език | готово | 2026-09-22 | `bansko.now/tv/<код>` → `/display/<token>` (`short_code`, `display_token_by_code()`); QR кодът е зелен с листо (`qr.ts`, H, проверен с jsQR); лепкавата лента на QR менюто е зелена с BG/EN; заглавия с шрифта на сайта, разделители между категориите |
> | 8a · Демо меню и „хартиен“ вид | готово | 2026-09-22 | демо кафето е преименувано на „Yum Spot“ (slug `yum-spot`) и носи истинското меню на Yum Spot (9 категории, 37 артикула, BG+EN); един вид за телевизор, QR и печат (`src/styles/menu-paper.css`: кремава хартия със зърно, кафяви линии, иконка на категорията, точки до цената); иконка по избор или по името (`business_menu_categories.icon_name`, `menu-icons.ts`); дълго меню се смалява само на телевизора |
> | 8c · Бранд тема | готово | 2026-09-22 | една тема на бизнес (`business_platform_settings.branding`, `theme.ts`): шест пресета, свои цветове, шрифтове, лого, орнамент; важи за QR менюто, телевизора и печата, не и за профила в bansko.now; `/business/branding`; телевизорът я получава от `display_by_token()`; лентата на QR менюто в тон с хартията, езикът фиксиран |
>
...
src/app/business/
  ...
  (portal)/
    ...
    branding/page.tsx       ← бранд: тема, цветове, шрифтове, лого, орнамент (от 2026-09-22)
...
src/lib/business-platform/{members,menu,media,displays,print,settings,theme}.ts
```
...
### Бранд тема (от 2026-09-22)

Всеки бизнес има **една тема** — цветове, шрифтове, лого и орнамент — и тя важи за трите му собствени изгледа: QR менюто, телевизора и листа за печат. Един и същ модел, три места, за да не се настройва три пъти. **Решение:** темата никога не пипа публичните страници на Bansko NOW — профилът `/places/<slug>`, `/places` и целият сайт остават във вида на сайта; брандът е за гостите на заведението, не за читателите на сайта.

Модел: `business_platform_settings.branding` (jsonb) `{ preset, accent, background, ink, heading_font, body_font, grain, ornament, logo_media_id }`. Шест пресета в `src/lib/business-platform/theme.ts`, всеки със светъл и тъмен набор цветове (`bg, ink, muted, accent, line, hair, badge, bar`): Хартия (по подразбиране, днешният вид), Гора, Мастило (тъмна по природа), Пясък, Маслина, Роза. Свои hex цветове важат за светлия вариант; приглушеният цвят и линиите се извеждат от тях, за да останат в един тон. Шрифтове: заглавен Georgia / Playfair Display / Lora / Inter / Montserrat, основен Inter / Montserrat / Lora — Lora и Montserrat са вградени (`public/fonts`, кирилица + латиница), на телевизора Georgia става Playfair. Логото е ред в `business_media` от вида `logo`.

Как стига до изгледите: `resolveTheme()` прави от jsonb-а валидна тема (невалидно → подразбиране, така че в стиловете и в SVG не влиза произволен текст), `themeTokens()` я превръща в CSS променливите `--paper-*` като inline style на `.paper` — `menu-paper.css` пази само подразбирането. QR менюто и печатът четат `getBusinessTheme()`; телевизорът получава `branding` + `logo` в отговора на `display_by_token()` и не пита базата отделно; смяната на темата вдига `content_version`, затова екраните се презареждат до минута. Нечетима комбинация (текст/фон < 4.5:1, акцент/фон < 3:1) не се записва. Портал: `/business/branding` — карти с темите (истинско парче меню), палитри с отметка „по темата“, лого, орнамент, преглед на първия екран. QR кодът в портала и на печата е в акцента на темата, ако е достатъчно тъмен на бяло.
...
**Вид (от 2026-09-22):** „хартиеното меню“ от `src/styles/menu-paper.css` — същото като на телевизора и на печат: кремава хартия със зърно (SVG feTurbulence в CSS, без картинка), заглавие с орнамент и разредени главни букви, иконка пред всяка категория (Font Awesome, `business_menu_categories.icon_name` или познато по името в `src/lib/business-platform/menu-icons.ts`), точки до цената, косъм между редовете, описанията в курсив. Общите парчета (заглавие, заглавие на категория) са в `src/components/public/paper-menu.tsx`. Цветовете, шрифтовете, зърното и орнаментът (листо / лого) идват от бранд темата на бизнеса (§8, „Бранд тема“); лепкавата лента с категориите е в тон с хартията (`--paper-bar`), превключвателят BG | EN стои неподвижно вляво и само категориите се плъзгат.
...
| Тема | `theme` | светла (хартия) | тъмна |
...
- Не се събира ли: текстът първо се смалява сам на стъпки до `--display-scale: 0.7` (от 2026-09-22; преди това се уголемяваше само нагоре). Ако и така прелива, страницата го знае (мери overflow) и го казва на прегледа в портала — „Не се събира на екрана: махни категория или скрий описанията“. Собственикът решава, не алгоритъм. Автоматичното превъртане на страници е Phase 2, ако някой клиент има 80 артикула.
...
| Един вид с телевизора и QR менюто | `menu-paper.css` (от 2026-09-22): хартия, иконки на категориите, точки, косъм между редовете; `print-menu.css` държи само листа, колоните и милиметрите |
