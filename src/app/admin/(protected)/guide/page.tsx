...
status: draft`}</CodeBlock>
          ...
          <h3 className="mt-6 font-semibold text-stone-950">Категории: видими и скрити</h3>
          <p className="mt-3">
            Категория без статии е скрита: няма я в менюто „Статии“, в sitemap и публичният ѝ адрес връща 404. Когда публикуваш първата статия в нея (от админа или през Content Hub), тя става видима автоматично. Можеш да включваш и изключваш категориите и ръчно от „Категории“ с отметката „Видима“. Празна, но видима категория остава извън индекса на Google (noindex), докато получи съдържание. Ако преместиш статия в друга категория, старият адрес пренасочва към новия.
          </p>
          <h3 className="mt-6 font-semibold text-stone-950">Снимки в три размера</h3>
          <p className="mt-3">
            Всяка нова снимка, качена през „Медия“ или дошла от Content Hub, се записва автоматично в три ширини (480, 960 и 1600 пиксела) като WebP. Картите и статиите зареждат най-малкия подходящ размер, което пести трафик и ускорява страниците на телефон. Старите снимки остават с един размер, докато не бъдат качени отново.
          </p>
        ...
        <GuideSection number="10" title="Art Studio каталог и поръчки">
          ...
          <h3 className="mt-6 font-semibold text-stone-950">Поръчка през форма (без предплащане)</h3>
          <p className="mt-3">
            Продуктовите типове (Тениски по поръчка, Fine Art принтове, Чаши и термоси, Икони) са продаващи страници с форма за поръчка. Клиентът избира опции (модел, размер, материал), брой, качва снимка по желание, оставя контакт и начин на получаване. Ти получаваш имейл с всичко и линк към снимката, клиентът получава потвърждение. Поръчката влиза в „Поръчки“ с тип „заявка“ и без предплащане; цената и срокът се потвърждават по телефон или имейл.
          </p>
          <p className="mt-3">
            Полетата на формата за всеки тип се задават в „Продукти“ → продуктов тип → „Форма за поръчка (JSON)“: списък <code>fields</code> с <code>key</code>, <code>label_bg</code>, <code>label_en</code>, <code>required</code> и <code>options</code>, плюс <code>photo_upload</code> (<code>optional</code>, <code>required</code> или <code>none</code>) и <code>quantity</code>. Продаващият текст на страницата е в полето „Продаващ текст“ (Markdown). Ако съдържа блок <code>:::faq</code>, въпросите заменят стандартните и получават FAQ схема за Google.
          </p>
          <p>
