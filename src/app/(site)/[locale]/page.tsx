...
function HeroMedia({ settings, locale }: { settings: SiteSettings; locale: "bg" | "en" }) {
...
export default async function HomePage({ params }: { params: Params }) {
  ...
  const todayCards = locale === "en"
    ? [
        { title: "Events", text: "Concerts, exhibitions and local initiatives.", href: "/events" },
        { title: "A place", text: "A walk through the Old Town and its quiet streets.", href: "/explore" },
        { title: "Weekend idea", text: "Pirin, a good view, warm tea and sturdy shoes.", href: "/nature" },
        { title: "Visual story", text: "Photography, light and moments from Bansko.", href: "/stories" }
      ]
    : [
        ...
        <section className="relative min-h-[100svh] overflow-hidden">
          ...
          <div className="relative mx-auto flex min-h-[100svh] max-w-7xl flex-col justify-end px-4 pb-16 pt-24 text-white sm:px-6 lg:px-8">
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={localePath(locale, "/now") as Route} className="rounded-full bg-white px-6 py-3 text-sm font-semibold text-stone-950 shadow-sm transition hover:bg-stone-100">
                {dictionary.heroToday}
              </Link>
              <Link
        ...
        <div className="mx-auto grid max-w-7xl gap-16 px-4 py-16 sm:px-6 lg:px-8">
          <section className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {todayCards.map((card) => (
                <Link key={card.title} href={localePath(locale, card.href) as Route} className="rounded-2xl border border-stone-200 bg-white p-5 shadow-soft">
