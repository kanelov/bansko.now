...
export default async function BusinessProfilePage({ params }: { params: Params }) {
  ...
  const [business, settings] = await Promise.all([getBusinessBySlug(slug, locale), getSiteSettings(locale)]);
        ...
        <div className="mx-auto grid max-w-5xl gap-12 px-4 py-16 sm:px-6 lg:px-8">
          ...
          {business.video_link ? (
            <section className="rounded-3xl border border-stone-200 bg-white p-6 shadow-soft">
              <p className="text-sm font-semibold uppercase text-moss">{locale === "en" ? "Video" : "Видео"}</p>
          ...
          {business.features?.length ? (
            <section>
              <p className="text-sm font-semibold uppercase text-moss">{locale === "en" ? "Features" : "Удобства"}</p>
          ...
          {faqs.length ? (
            <section>
              <p className="text-sm font-semibold uppercase text-moss">{locale === "en" ? "Questions" : "Въпроси"}</p>
