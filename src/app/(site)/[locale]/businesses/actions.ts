...
export async function submitBusinessAction(formData: FormData) {
  ...
  if (!supabase) {
    redirect(`${submitPath}?error=missing-env`);
  }
  ...
  if (!name || !category || !address || !ownerName || !ownerEmail) {
    redirect(`${submitPath}?error=missing-fields`);
  }
  ...
  if (result.error || !result.data?.id) {
    redirect(`${submitPath}?error=${encodeURIComponent(result.error?.message || "submission-failed")}`);
  }

  await supabase.from("business_translations").insert({
  ...
  redirect(`${submitPath}?submitted=1`);
}
...
export async function submitContactMessageAction(formData: FormData) {
  ...
  if (!supabase) {
    redirect(`${contactPath}?error=missing-env`);
  }
  ...
  if (!name || !email || !message) {
    redirect(`${contactPath}?error=missing-fields`);
  }
  ...
  if (error) {
    redirect(`${contactPath}?error=${encodeURIComponent(error.message)}`);
  }
