...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const page = await getEditablePageBySlug("about", { locale });
  ...
  return {
    ...
    alternates: {
      canonical: page?.canonical_url || localeUrl(locale, "/about"),
      languages: { bg: localeUrl("bg", "/about"), en: localeUrl("en", "/about"), "x-default": localeUrl("bg", "/about") }
    },
    openGraph: {
...
export default async function AboutPage({ params }: { params: Params }) {
  ...
  const [settings, page] = await Promise.all([getSiteSettings(locale), getEditablePageBySlug("about", { locale })]);

  return (
    <div>
      <SiteHeader locale={locale} alternateHref={localePath(locale === "bg" ? "en" : "bg", "/about")} />
      <main className="mx-auto grid max-w-5xl gap-10 px-4 py-16 sm:px-6 lg:px-8">
        <header className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
          <div>
            <p className="text-sm font-semibold uppercase text-moss">{page?.eyebrow || dictionary.about}</p>
            ...
            <p className="mt-6 text-xl leading-9 text-stone-650">
              {page?.excerpt || dictionary.heroText}
            </p>
          </div>
          {page?.hero_image_url ? (
