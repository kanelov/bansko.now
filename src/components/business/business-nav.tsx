...
const items: { href: string; label: string; ready: boolean }[] = [
  ...
  { href: "/business/qr", label: "QR код", ready: true },
...
export function BusinessNav({ variant }: { variant: "side" | "bottom" }) {
  ...
  return (
    <nav
      aria-label="Портал"
      className="fixed inset-x-0 bottom-0 z-20 border-t border-[var(--stone)] bg-white/95 pb-[env(safe-area-inset-bottom)] backdrop-blur print:hidden lg:hidden"
    >
