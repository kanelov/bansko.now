...
/**
 * Петте места в портала. Модул без страница още стои в лентата като „скоро“,
 * за да се вижда какво идва, но не води наникъде.
 */
        ...
        {items.map((item) => (
          <li key={item.href} className="flex">
            {item.ready ? (
              ...
              <span className="flex flex-1 flex-col items-center gap-0.5 px-1 py-3 text-xs text-stone-400">
                <span aria-hidden className="h-1 w-6 rounded-full bg-transparent" />
                {item.label}
              </span>
            )}
          </li>
