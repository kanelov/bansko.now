import { getArticleBlocks, isArticleToggleKey, manualBlockPlacement, renderArticleBlock } from "@/lib/article-blocks";
 ...
 * custom blocks without a switch appear under every article while they are active; blocks marked
 * "manual" appear only where the text places them with :::block.
 */
export async function ArticleBlocks({ article, locale, settings }: { article: ArticleWithCategory; locale: Locale; settings?: Pick<SiteSettings, "facebook_group_url"> | null }) {
  ...
  const visible = blocks.filter((block) => {
    ...
    if (block.article_toggle === manualBlockPlacement) return false;
