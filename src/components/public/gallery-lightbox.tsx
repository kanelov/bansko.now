...
function bypassImageOptimization(src: string) {
  return src.includes(".supabase.co/storage/v1/object/");
}
            ...
            <figure key={`${image.src}-${index}`} className="grid gap-3" itemScope itemType="https://schema.org/ImageObject">
              ...
              >
                <Image
                  ...
                  unoptimized={bypassImageOptimization(image.src)}
      ...
      {activeImage ? (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/85 p-4" role="dialog" aria-modal="true" aria-label={locale === "en" ? "Gallery" : "Галерия"}>
          ...
          <figure className="grid max-h-[90vh] max-w-6xl gap-3" itemScope itemType="https://schema.org/ImageObject">
            <Image src={activeImage.src} alt={activeImage.alt} width={1600} height={1200} sizes="100vw" unoptimized={bypassImageOptimization(activeImage.src)} className="max-h-[82vh] w-auto max-w-[calc(100vw-2rem)] rounded-2xl object-contain" itemProp="contentUrl" />
