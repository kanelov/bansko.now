...
- **Demo café menu (2026-09-22).** The demo „Old Town Coffee“ carries the real menu of the Yum Spot café (two photographed pages): 9 categories, 37 items, BG + EN names, descriptions on the ciabattas and filled croissants, all linked to the demo display in menu order. It was written straight into the database (script kept outside the repo), so the ISR cache of the QR menu was refreshed by the next deployment. The public footer „Връзки“ column links to `/business` („За бизнеси“ / "For businesses", dictionary key `businessPortal`).
