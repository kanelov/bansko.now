...
- **QR menu, `/places` and hours (step 4, 2026-09-16).** Migrations `…154905_business_hours.sql` (`business_hours`: one row per open interval, ISO weekday 1–7, a day without rows is closed; `business_hour_exceptions`: one row per date, `is_closed` or explicit times, both bump `content_version`; public read through `public_business_ids('hours')`, owners write their own) and `…155027_business_platform_settings_public_read.sql` (anon reads `open_override`/`content_version` of active businesses — the existing policy was authenticated only). `src/lib/business-platform/hours.ts` holds the whole open/closed rule: intervals per weekday, exceptions beat the week, `open_override` beats everything, the clock is always Europe/Sofia (`Intl.DateTimeFormat`, no date library), intervals past midnight belong to the day they start on, plus `groupWeek()` for display and `openingHoursSpecification()` for schema.org.
- **Public catalog renamed to `/places` (2026-09-16).** `src/app/(site)/[locale]/businesses` → `places`; `/businesses/:path*` and `/en/businesses/:path*` are 308 redirects in `next.config.ts`; the `navigation_items` row was repointed by `…160000_places_navigation_href.sql`. Use `/places` in every new link. The profile now exports `revalidate = 900` (it used to hit Supabase on every request) and shows a menu excerpt, the hours and an open/closed badge; its JSON-LD carries `hasMenu`, `openingHoursSpecification` and `telephone`.
- **QR menu page** `src/app/(site)/[locale]/places/[slug]/menu/page.tsx` (ISR 900, zero client JavaScript, anchors instead of tabs, `Menu`/`MenuSection`/`MenuItem` JSON-LD with EUR offers, self-canonical + hreflang only when the other locale is published, 404 when the business has no public menu). Public reads go through `getPublicBusinessPlatform()` in `src/lib/business-platform/public-business.ts`; there is no "is the module on" check in code — RLS already hides menu and hours of a business whose module is off, so an empty result *is* the answer. The same file has `revalidateBusinessPublicPages()` (called by every portal action, both locale slugs) and `getBusinessIdsWithPublicMenu()` for the sitemap, which now lists the menu URLs too.
- **Portal pages added:** `/business/hours` (week with two shifts per day + exceptions by date; actions in `src/app/business/hours/actions.ts` rewrite the whole week in one delete+insert), `/business/qr` (server-rendered SVG through the new `qrcode` dependency, `qrMenuUrl()` adds `?src=qr` for later analytics, „Отпечатай“ prints the page — the shell is `print:hidden`) and `/business/guide` (owner help, „Накратко“/„Подробно“, its own components; keep it current with every flow change — that is the fourth documentation place).
