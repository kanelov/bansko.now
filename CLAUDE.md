...
- **Pending from step 5:** the demo display seed (`…150100_business_displays_demo_seed.sql`) was **not applied** to the live database (the Supabase MCP refused every call from 2026-09-16 evening); `/display/<token>` was verified only for unknown tokens (404) and as a static mock with the real CSS and script. Apply the seed (or create a display in the portal) and open the page once before the pilot.
